...
public class MoneyBoxServiceTest {
    ...
    @ParameterizedTest
    @ArgumentsSource(AddNotesArgumentsProvider.class)
    ...
    static class AddNotesArgumentsProvider implements ArgumentsProvider {
        @Override
        public Stream<? extends Arguments> provideArguments(ExtensionContext context) {
            return Stream.of(
                    Arguments.of(1, 0, 0, 0, 100),
                    Arguments.of(0, 1, 0, 0, 500),
                    Arguments.of(0, 0, 1, 0, 1000),
                    Arguments.of(0, 0, 0, 1, 5000),
                    Arguments.of(1, 1, 1, 1, 6600),
                    Arguments.of(2, 2, 2, 2, 13200),
                    Arguments.of(10, 10, 10, 10, 66000)
            );
        }
    }

    @Test
    void addNotesCheckForMoneyBoxNull() {
        Exception thrown = assertThrows(IllegalStateException.class, () -> {
            moneyBoxService.putMoney(null, 1, 1, 1, 1);
        });
        assertEquals("No money box", thrown.getMessage());
    }
}
