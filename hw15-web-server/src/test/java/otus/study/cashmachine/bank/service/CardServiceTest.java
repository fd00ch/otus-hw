...
public class CardServiceTest {
    ...
    @Test
    void checkBalanceIfCardIsNull() {
        when(cardsDao.getCardByNumber(any())).thenReturn(null);

        Exception thrown = assertThrows(IllegalArgumentException.class, () -> {
            cardService.getBalance("1234", "9876");
        });
    ...
    @Test
    void getMoneyIfCardIsNull() {
        when(cardsDao.getCardByNumber(any())).thenReturn(null);

        Exception thrown = assertThrows(IllegalArgumentException.class, () -> {
            cardService.getMoney("1234", "9876", BigDecimal.ONE);
        });
        assertEquals("No card found", thrown.getMessage());
    }
    ...
    @Test
    void putMoneyBoxIfCardIsNull() {
        when(cardsDao.getCardByNumber(any())).thenReturn(null);

        Exception thrown = assertThrows(IllegalArgumentException.class, () -> {
            cardService.putMoney("1234", "9876", BigDecimal.ONE);
        });
        assertEquals("No card found", thrown.getMessage());
    }
    ...
    @Test
    void changePinIfCardIsNull() {
        when(cardsDao.getCardByNumber(any())).thenReturn(null);

        Exception thrown = assertThrows(IllegalArgumentException.class, () -> {
            cardService.changePin("1234", "9876", "0000");
        });
        assertEquals("No card found", thrown.getMessage());
    }
}